...
$(document)
    .on("keyup", ".input_sicon", function () {
        ...
        $(".input_sicon").each(function (e) {
            ...
            if (val != "") {
                html +=
                    '<div class="col-3" style=""><div class="icon"><a href="' +
                    val +
                    '" target="_blank" class="sicon_facebook_url"><i class="' +
                    icon_fa +
                    '"></i><span>' +
                    icon_title +
                    "</span></a></div></div>";
...
$(document).on("click", ".social_icon", function () {
    ...
    var html =
        '<div class="col-12 sicon_' +
        icon_name +
        ' drag-handler"><div class="input-group mb-2"><span class="input-group-text"><i class="fa fa-bars text-default" aria-hidden="true"></i><i class="' +
        icon_fa +
        '"></i></span><input type="text" class="form-control input_sicon" data-icon_name="' +
        icon_name +
        '" data-icon_fa="' +
        icon_fa +
        '" data-icon_title="' +
        ...
        '[]" placeholder="' +
        example_text +
        '"><span class="input-group-text"><i class="fa fa-times text-default remove_sicon" data-delclass="' +
        icon_name +
...
$(document).on(
    ...
    function (e) {
        ...
        var arr_cont = $(".testimonial_content")
            .map((i, e) => e.value)
            .get();
        var arr_name = $(".testimonial_name")
            .map((k, l) => l.value)
            .get();
        var html = "";
        for (var i = 0, l = arr_cont.length; i < l; i++) {
            html +=
                '<div class="item"><div class="content"><p>' +
                arr_cont[i] +
                "</p><h3>" +
                arr_name[i] +
                "</h3></div></div>";
...
$(document).on(
    "keyup change",
    ".video_title, .video_type, .video_file, .video_description",
    function (e) {
        $(".video_sec").addClass("video_sec_");
        var arr_video_title = $(".video_title")
            .map((i, e) => e.value)
            .get();
        var arr_video_type = $(".video_type")
            .map((k, l) => l.value)
            .get();
        var arr_video_file = $(".video_file")
            .map((m, n) => n.value)
            .get();
        var arr_video_src = $(".video_src")
            .map((m, n) => n.value)
            .get();
        var arr_video_description = $(".video_description")
            .map((x, y) => y.value)
            .get();
        var html = "";
        var video_tag = "";
        for (var i = 0, l = arr_video_title.length; i < l; i++) {
            ...
            if (arr_video_type[i] == "link") {
                ...
                video_tag =
                    ...
                    file +
                    '" title="" frameborder="0" allow="accelerometer; encrypted-media; gyroscope;" allowfullscreen></iframe>';
            } else {
                file = arr_video_src[i];
                console.log(arr_video_src[i]);
                video_tag =
                    '<video height="200" src="' +
                    file +
                    '" width="100%" class="video-preview" controls="controls"/>';
            }
            html +=
                '<div class="video_sec_child mb-4"><div class="video_content"><div class="title"><h3>' +
                arr_video_title[i] +
                '</h3></div><div class="video mb-3 ">' +
                video_tag +
                '</div><div class="video_description">' +
                arr_video_description[i] +
                "</div></div></div>";
        }
...
$(document).on("click", ".remove_sicon", function (e) {
    ...
    $(this)
        .closest(".sicon_" + delclass)
        .remove();
});

$(document)
    .on("input", "#personalized_link", function () {
...
$(document).on("click", ".onclickIcon", function () {
    ...
    var html =
        '<li class="sicon_' +
        id +
        '"><a class="social_link" href="" target="_blank"><img style="background:' +
        color +
        '" src="' +
        image +
        '" alt="email" class="social_logo"><span class="icon_label link_title_show">' +
        title +
        "</span></a></li>";
