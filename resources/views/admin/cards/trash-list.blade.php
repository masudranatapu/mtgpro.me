                        ...
                        <div class="card-header">
                            <div class="col">
                                <div class="float-left">
                                    <a href="{{ route('admin.cards') }}" class="@yield('card_list')"><i class="fa fa-user"></i> {{ __('All Cards') }}</a>  |
                                    <a href="{{ route('admin.card.trash') }}" class="@yield('trash_card')"> <i class="fa fa-trash-alt"></i> Trash list</a>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            <div class="px-2 py-2">
                                <table class="table table-vcenter card-table" id="table-plan" style="width: 100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{ $loop->index + 1 }}</td>
                                            <td><a href="{{ route('admin.view.user', $row->user_id)}}">{{ $row->title }} {{ $row->title2 }}</a></td>
                                            <td class="text-muted">
                                                {{ $row->card_email }} <br>
                                                {{ $row->phone_number }}
                                            </td>
                                            ...
                                            <td>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item btn-sm" href="{{ route('card.preview',$row->card_url) }}">{{ __('View') }}</a>
                                                        <a class="dropdown-item btn-sm"onclick="if (confirm('You want to active?')){return true;}else{event.stopPropagation(); event.preventDefault();};" href="{{ route('admin.card.active',$row->id) }}" >{{ __('Active') }}</a>
                                                        <a class="dropdown-item btn-sm" onclick="if (confirm('You want to premanent delete?')){return true;}else{event.stopPropagation(); event.preventDefault();};" href="{{ route('admin.card.delete',$row->id) }}" >{{ __('Premanent Delete') }}</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </tr>
